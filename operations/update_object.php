...
foreach ($obj_decoded as $this_obj_num=>$this_obj) {
	...
	$this_obj_t = $db_connection->query("SELECT s1.id, s1.type, s1.content, s1.parent, s1.width, s1.sort_order, s1.published, s1.deleted FROM objects s1 LEFT JOIN objects s2 ON s1.id = s2.id AND s1.time < s2.time WHERE s2.id IS NULL AND s1.id = '".$new_obj_array[$this_obj_num]['id']."'");
	$this_obj_t_result = $this_obj_t->fetch_assoc();
	$new_obj_array[$this_obj_num]['type'] = $this_obj_t_result['type'];
	$new_obj_array[$this_obj_num]['content'] = $this_obj_t_result['content'];	
	$new_obj_array[$this_obj_num]['parent'] = $this_obj_t_result['parent'];	
	$new_obj_array[$this_obj_num]['width'] = $this_obj_t_result['width'];
	$new_obj_array[$this_obj_num]['sort_order'] = $this_obj_t_result['sort_order'];
	$new_obj_array[$this_obj_num]['published'] = $this_obj_t_result['published'];
	$new_obj_array[$this_obj_num]['deleted'] = $this_obj_t_result['deleted'];
